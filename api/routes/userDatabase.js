...
function updateUser(collection, req) {
  ...
  if (req.body.firstName != "")
    collection.updateOne(query, { $set: { firstName: req.body.firstName } });
  if (req.body.lastName != "")
    collection.updateOne(query, { $set: { lastName: req.body.lastName } });
  if (req.body.link1 != "")
    collection.updateOne(query, { $set: { link1: req.body.link1 } });
  if (req.body.link2 != "")
    collection.updateOne(query, { $set: { link2: req.body.link2 } });
  if (req.body.link3 != "")
    collection.updateOne(query, { $set: { link3: req.body.link3 } });
  if (req.body.workExperience != "")
    collection.updateOne(query, {
      $set: { workExperience: req.body.workExperience }
    });
  if (req.body.notes != "")
    collection.updateOne(query, { $set: { notes: req.body.notes } });
}
